...
import { formatSeconds, truncateMiddle } from "../../ui/format.js";
...
function renderNowPlaying(state: VisState, renderer: Renderer, r: Region): void {
  const { x, y, width: W } = r;
  const song = state.songTheme;

  if (state.trackName) {
    const title  = truncateMiddle(`${state.trackName}  \u2014  ${state.artistName}`, Math.max(1, W - 4));
    renderer.write(x + 2, y,     st(title, song.normal, song.reset));
    const sub    = truncateMiddle(state.albumName, Math.max(1, W - 4));
...
function renderKnobs(state: VisState, renderer: Renderer, r: Region): void {
  const { x, y, width: W } = r;
  const song = state.songTheme;
  const left  = "\u25C9 tempo";
  const right = "\u25C9 volume";
...
function renderTurntable(state: VisState, renderer: Renderer, r: Region): void {
  const song = state.songTheme;
...
function renderTonearm(state: VisState, renderer: Renderer, r: Region): void {
  const song = state.songTheme;
  const cx   = r.x + Math.floor(r.width  / 2);
  const cy   = r.y + Math.floor(r.height / 2);
  const rX   = Math.max(4, Math.floor(r.width  * 0.42));
...
function renderMidProgress(state: VisState, renderer: Renderer, r: Region): void {
  const { x, y, width: W } = r;
  if (W < 6) return;
  const song    = state.songTheme;
...
function renderMidGrille(state: VisState, renderer: Renderer, r: Region, now: number): void {
  const { x, y, width: W } = r;
  if (W < 2) return;
  const song    = state.songTheme;
...
function renderAlbumArtScreen(state: VisState, renderer: Renderer, r: Region): void {
  const song = state.songTheme;
  ...
  if (art && art.playerLines.length > 0) {
    ...
    renderer.write(
      ix + Math.max(0, Math.floor((iW - mark.length) / 2)),
      iy + Math.floor(iH / 2),
      st(mark, song.dim, song.reset)
    );
  }
}
...
export function renderRecordDeck(
  state: VisState,
  renderer: Renderer,
  region: Region,
  now: number
  ...
  const [nowR, , knobsR, platterR, , midR, artR] = vSplit(inner, [
    C.length(2),         // now-playing strip
    C.length(1),         // gap
    C.length(1),         // knobs row
    C.length(platterH),  // platter area
    C.length(1),         // gap before mid
    C.length(1),         // mid-divider row
    C.fill(),            // album art (remaining)
  ]);
  ...
  if (midR && midR.height > 0) {
    const [progR, sepR, grilleR] = hSplit(midR, [
      C.percent(52),
      C.length(1),
      C.fill(),
    ]);
    renderMidProgress(state, renderer, progR);
    if (sepR) renderer.write(sepR.x, sepR.y, st("\u2502", song.dim, song.reset));
