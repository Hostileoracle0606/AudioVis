...
import { computeDna, catalogNumber, type TrackDna } from "../trackDna.js";
import { blinkColor } from "../theme.js";
import { renderMiniLine, renderTinyLine, GLYPH_W } from "../../ui/bitfont.js";
...
function gauge(value: number, width: number, color: string, theme: Theme): string {
...
function drawMiniMarquee(
  ...
  const rows = renderTinyLine(body);
...
function dnaInfoLine(dna: TrackDna, theme: Theme): string {
  return (
    `${COLOUR.label}bpm ${COLOUR.bpm}${String(dna.bpm).padStart(3, " ")}${theme.reset}` +
...
export function renderNowPlaying(
  ...
  const hdrColor = np ? blinkColor(theme, Date.now()) : theme.dim;
  ...
  const gaugeCell = (lbl: string, val: number, color: string) =>
    `${COLOUR.label}${lbl}${theme.reset} ${gauge(val, gaugeW, color, theme)} ${color}${String(val).padStart(2, " ")}${theme.reset}`;
...
export function stepScope(
  ...
  for (let i = 0; i < samplesPerFrame; i++) {
    ...
    if (scopeWriteIdx === 0) scopeFilled = true;
  }
}
...
function drawDotMatrixScope(
  ...
  for (let row = 0; row < height; row++) grid.push(new Uint16Array(width));

  const lit = (px: number, py: number): void => {
    if (py < 0 || py >= pxRows || px < 0 || px >= pxCols) return;
    const col = px >> 1;
    const pCol = px & 1;
    const row = py >> 2;
    const pRow = py & 3;
  ...
  for (let row = 0; row < height; row++) {
    for (let col = 0; col < width; col++) {
      ...
      if (bits === 0) {
        ...
        if (onCrosshair) r.write(x + col, y + row, `${theme.dim}\u00B7${theme.reset}`);
        else r.write(x + col, y + row, " ");
      } else {
