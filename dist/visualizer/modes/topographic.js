...
exports.renderTopographic = renderTopographic;
const pitchPalette_js_1 = require("../pitchPalette.js");
const CONTOUR_RAMP = [" ", ".", "-", ":", "=", "+", "*", "#"];
function clamp01(value) {
    return Math.max(0, Math.min(1, value));
}
function sampleBuckets(values, position) {
    if (values.length === 0)
        return 0;
...
function renderTopographic(state, renderer, region, theme) {
    ...
    for (let x = 0; x < W; x++) {
        ...
        for (let y = 0; y < H; y++) {
            ...
            const warp = 0.05 * Math.sin(ny * TAU * (2.1 + state.high * 2.3) + now * (0.7 + state.high * 0.35)) +
            ...
            const charIndex = Math.max(1, Math.min(CONTOUR_RAMP.length - 1, Math.floor(density * (CONTOUR_RAMP.length - 1))));
...
//# sourceMappingURL=topographic.js.map