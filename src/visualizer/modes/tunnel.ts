...
function createRing(seed: number): TunnelRing {
  return {
    depth: 0.08,
...
function getTunnelModeData(state: VisState, width: number, height: number): TunnelModeData {
  const existing = state.modeData.tunnel as TunnelModeData | undefined;
...
export function prepareTunnel(state: VisState, region: Region): void {
