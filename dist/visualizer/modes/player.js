...
// ── Helpers ───────────────────────────────────────────────────────────────
function styled(s, colour, reset) {
    return colour ? colour + s + reset : s;
}
function drawRoundedBox(renderer, region, label, colour, reset) {
    ...
    const topInner = "\u2500".repeat(leftDash) + labelTxt + "\u2500".repeat(rightDash);
 ...
 */
function clearRegion(renderer, region, resetCell) {
...
function clampRegion(inner, parent) {
...
function renderBackgroundPlate(state, renderer, region, theme) {
    const dimTheme = {
        ...theme,
    ...
    switch (state.backgroundMode) {
        case "scroll":
            (0, scroll_js_1.renderScroll)(dimState, renderer, region, dimTheme);
            break;
        case "spectrum":
            (0, spectrum_js_1.renderSpectrum)(dimState, renderer, region, dimTheme);
            break;
        default:
            (0, wavefield_js_1.renderWavefield)(dimState, renderer, region, dimTheme);
            break;
    }
}
// ── Turntable ─────────────────────────────────────────────────────────────
function renderTurntable(state, renderer, region) {
    ...
    for (let i = 0; i < STEPS; i++) {
        ...
        if (px < region.x || px >= region.x + region.width)
            continue;
        if (py < region.y || py >= region.y + region.height)
            continue;
    ...
    if (labelW >= 8 && labelH >= 3) {
        ...
        const labelRegion = { x: lx, y: ly, width: labelW, height: labelH };
        ...
        const lines = [
            (0, format_js_1.centerPad)("side a", innerW).slice(0, innerW),
            (0, format_js_1.centerPad)(`\u2756 \u00B7 33\u2153`, innerW).slice(0, innerW),
            (0, format_js_1.centerPad)(`track ${trackNum}`, innerW).slice(0, innerW),
        ];
...
// ── Tonearm ───────────────────────────────────────────────────────────────
function renderTonearm(state, renderer, region) {
    const song = state.songTheme;
    ...
    const progress = state.durationMs > 0 ? Math.max(0, Math.min(1, state.progressMs / state.durationMs)) : 0;
...
// ── Screen (album art) ────────────────────────────────────────────────────
function renderScreen(state, renderer, region) {
    const song = state.songTheme;
...
// ── Grille ────────────────────────────────────────────────────────────────
function renderGrille(state, renderer, region, now) {
    const song = state.songTheme;
...
// ── Progress strip ────────────────────────────────────────────────────────
function renderProgressStrip(state, renderer, region) {
    const song = state.songTheme;
    const trackNum = state.currentTrackId
        ? String((0, songTheme_js_1.hashTrackId)(state.currentTrackId) % 1000).padStart(3, "0")
        : "000";
...
// ── Controls ──────────────────────────────────────────────────────────────
function renderControls(state, renderer, region) {
    const song = state.songTheme;
...
// ── Chassis composition ───────────────────────────────────────────────────
function renderChassis(state, renderer, region, now) {
    const song = state.songTheme;
    const trackNum = state.currentTrackId
        ? String((0, songTheme_js_1.hashTrackId)(state.currentTrackId) % 1000).padStart(3, "0")
        : "042";
    ...
    if (midH >= 10) {
        ...
        const platterRegion = { x: ix, y: midTop, width: iw, height: platterH };
...
// ── Side module ───────────────────────────────────────────────────────────
function ensureLyricsLines(state) {
    ...
    if (state.lyricsLines.length === 0 || state.lyricsLines[0] !== state.trackName) {
        const lines = [];
        if (state.trackName)
            lines.push(state.trackName);
        if (state.artistName)
            lines.push(state.artistName);
        if (state.albumName)
            lines.push(state.albumName);
        lines.push(state.isPlaying ? "now playing" : "paused");
        if (state.deviceName)
            lines.push(`\u00B7 ${state.deviceName}`);
        state.lyricsLines = lines.length > 0 ? lines : ["audio\u00B7vis"];
    }
    return state.lyricsLines;
}
function renderLyricsPanel(state, renderer, region, now) {
    const song = state.songTheme;
    ...
    for (let i = 0; i < Math.min(innerH, lines.length); i++) {
        ...
        const text = (0, format_js_1.truncateMiddle)(lines[idx], innerW - 2);
...
function renderWavePanel(state, renderer, region) {
    const song = state.songTheme;
    ...
    for (let c = 0; c < innerW; c++) {
        ...
        if (row >= innerY && row < innerY + innerH) {
            ...
            let colour = song.dim;
            if (Math.abs(mixed) > 0.55)
                colour = song.bright || song.normal;
            else if (Math.abs(mixed) > 0.25)
                colour = song.normal;
...
function renderSideModule(state, renderer, region, now) {
    if (region.height < 8) {
...
// ── Search field ──────────────────────────────────────────────────────────
function renderSearchField(state, renderer, region) {
    const song = state.songTheme;
    ...
    const shown = (0, format_js_1.truncateMiddle)(text, Math.max(1, innerW - caret.length));
...
function tickPlayerState(state, now) {
...
// ── Entry point ───────────────────────────────────────────────────────────
function renderPlayer(state, renderer, region, theme) {
    const now = Date.now();
    tickPlayerState(state, now);
    const layout = (0, layout_js_1.computePlayerLayout)(region);
