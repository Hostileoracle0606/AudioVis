...
exports.wrapLyric = wrapLyric;
exports.renderBigLyric = renderBigLyric;
 ...
 */
function wrapLyric(text, maxChars) {
    if (maxChars <= 0)
        return [];
 ...
 */
function renderBigLyric(r, region, props, theme) {
    if (region.width < 8 || region.height < BIG_ROWS_PER_LINE)
        return;
    ...
    const maxChars = Math.max(0, Math.floor((textMaxWidth + 1) / (bitfont_js_1.GLYPH_W + 1)));
    if (maxChars === 0)
        return;
    ...
    for (let li = 0; li < wrapped.length; li++) {
        ...
        const pixelRows = (0, bitfont_js_1.renderBigLine)(lineText);
        const compressed = (0, bitfont_js_1.compressToHalfBlock)(pixelRows);
        for (let i = 0; i < compressed.length && i < BIG_ROWS_PER_LINE; i++) {
            let line = compressed[i];
            if (line.length > textMaxWidth)
                line = line.slice(0, textMaxWidth);
...
//# sourceMappingURL=bigLyric.js.map