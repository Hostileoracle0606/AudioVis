...
function normaliseDb(value: number, lo: number, hi: number): number {
  return clamp01((value - lo) / (hi - lo));
}

function computeGenreBias(artists: SpotifyArtist[]): Record<string, number> {
  const scores = {
    glitch: 0,
    neon: 0,
    organic: 0,
...
function computeDominantPitch(analysis: SpotifyAudioAnalysis | null): {
  dominantPitchClass: number;
  hue: number;
} {
  ...
  const pitchTotals = new Array<number>(12).fill(0);
...
function computeAnalysisTraits(analysis: SpotifyAudioAnalysis | null): {
  glitch: number;
  metallic: number;
  density: number;
  aggression: number;
  ...
  const loudnessSpread = average(
    segments.map((segment) => segment.loudness_max - segment.loudness_start)
  );
  const sectionDensity =
    analysis.sections.length > 0 ? analysis.segments.length / analysis.sections.length : 0;
  const tatumRate = analysis.track.duration > 0 ? analysis.tatums.length / analysis.track.duration : 0;

  return {
    glitch: clamp01(
      normaliseDb(loudnessSpread, 2, 16) * 0.55 +
        normaliseDb(timbre7, -80, 80) * 0.25 +
        clamp01(tatumRate / 6) * 0.2
    ),
    metallic: clamp01(
      normaliseDb(timbre5, -80, 80) * 0.45 +
        normaliseDb(timbre6, -80, 80) * 0.35 +
        normaliseDb(timbre2, -80, 80) * 0.2
    ),
    density: clamp01(
      clamp01(sectionDensity / 30) * 0.5 + clamp01(tatumRate / 6) * 0.5
    ),
    aggression: clamp01(
      normaliseDb(loudnessMax, -30, 0) * 0.6 +
        normaliseDb(timbre0, 0, 60) * 0.4
    ),
    softness: clamp01(
      0.6 * (1 - normaliseDb(timbre7, -80, 80)) +
        0.4 * (1 - normaliseDb(loudnessSpread, 2, 16))
    ),
  };
}

function pickLabel(profile: Omit<StyleProfile, "label" | "confidence" | "genreHints">): string {
  const parts: string[] = [];
  if (profile.glitch > 0.62) parts.push("glitch-charged");
  else if (profile.organic > 0.62) parts.push("organic");
  else if (profile.groove > 0.58) parts.push("groove-led");
  else parts.push("shape-shifting");

  if (profile.neon > 0.6) parts.push("neon");
  else if (profile.darkness > 0.62) parts.push("nocturne");
  else if (profile.softness > 0.62) parts.push("dreamy");
  else if (profile.metallic > 0.58) parts.push("chrome");

  return parts.slice(0, 2).join(" ");
}

export function inferStyleProfile(args: {
...
export function smoothStyleProfile(
...
function spanProgress(start: number, duration: number, progressMs: number): number {
  if (duration <= 0) return 0;
  return clamp01((progressMs / 1000 - start) / duration);
}

export function buildAnalysisFrame(args: {
  analysis: SpotifyAudioAnalysis | null;
  progressMs: number;
  ...
  return {
    ...
    sectionProgress,
    sectionTransition:
      sectionProgress < 0.14 ? 1 - sectionProgress / 0.14 :
