...
exports.renderLyricsTerminal = renderLyricsTerminal;
...
function renderLyricsTerminal(state, renderer, region, now) {
    ...
    if (state.lrcLines.length === 0) {
        ...
        switch (state.lyricFetchState) {
            case "fetching":
                msg = "loading lyrics\u2026" + cursor;
                break;
            case "none":
                msg = "no lyrics found " + cursor;
                break;
            default: msg = "> " + cursor;
        }
    ...
    const revealed = Math.min(lines[active]?.text.length ?? 0, Math.floor(ageMs * CHARS_PER_SEC / 1000));
    ...
    for (let i = firstLineIdx; i <= lastLineIdx; i++) {
        ...
        if (rowY >= region.y + region.height - 1)
            break;
        const lyric = lines[i];
        if (!lyric)
            continue;
...
//# sourceMappingURL=lyricsTerminal.js.map