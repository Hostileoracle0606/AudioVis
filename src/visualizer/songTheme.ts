...
export interface SongTheme {
  ...
  normal: string;
  bright: string;
  accent: string;       // a secondary colour for beat flashes, rings, sparkles
  reset: string;
  ...
  phaseSpeedMul: number;   // wavefield plasma + scroll sweep speed
  waveLayerBias: number;   // extra sine layers in wavefield ASCII path
  spatialFreqMul: number;  // spatial detail / busyness
...
const GLYPH_FAMILIES: ReadonlyArray<{
  ramp: string[];
  ring: string;
  sparkle: string;
  peak: string;
...
const HUE_TABLE: ReadonlyArray<{ name: string; tier: [number, number, number, number] }> = [
...
function sgr256(idx: number, style = ""): string {
...
export function buildSongTheme(
  trackId: string,
  features: SongFeatures,
  fallback: BaseThemeFallback,
  ...
  let hueIdx = (h >>> 8) & 0x07;
  if (features.brightness > 0.55) hueIdx = [1, 2, 5, 7][(h >>> 16) & 3];
  else if (features.warmth > 0.55) hueIdx = [3, 4, 5][(h >>> 16) % 3];
  ...
  const phaseSpeedMul =
    0.7 + Math.min(1.3, bpmRef / 120) * 0.6 + features.dynamics * 0.3;
...
export function defaultSongTheme(fallback: BaseThemeFallback, colorEnabled: boolean): SongTheme {
