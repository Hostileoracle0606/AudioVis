...
exports.pushScrollHistory = pushScrollHistory;
exports.renderScroll = renderScroll;
const MAX_PARTICLES = 140;
function pushScrollHistory(state, width) {
...
function renderScroll(state, renderer, region, theme) {
    ...
    const now = Date.now();
    const dt = Math.min(1 / 15, state.lastScrollMs
        ? (now - state.lastScrollMs) / 1000
        : 1 / 30);
    ...
    while (spawnsLeft > 0 && state.particles.length < MAX_PARTICLES) {
        if (spawnsLeft < 1 && rand() > spawnsLeft)
            break;
        spawnsLeft -= 1;
    ...
    for (const p of state.particles) {
        ...
        if (theme.colorEnabled) {
            if (p.life > 0.7)
                cell = song.accent + p.glyph + song.reset;
            else if (p.life > 0.35)
                cell = song.bright + p.glyph + song.reset;
            else
                cell = song.dim + p.glyph + song.reset;
        }
        renderer.write(col, region.y + row, cell);
    }
    ...
    if (beatFlash > 0.05 && theme.colorEnabled) {
        ...
        if (beatFlash > 0.35 && !ascii) {
            ...
            for (let col = 0; col < W; col += 2) {
                ...
                if (r1 >= region.y)
                    renderer.write(col, r1, echoCell);
                if (r2 < region.y + H)
                    renderer.write(col, r2, echoCell);
            }
        }
    }
}
