...
function stereoSeparator(label: string, width: number): string {
  const inner = ` ${label} `;
...
export function renderNowPlaying(
  ...
  const renderMeter = (label: string, level: number, y: number) => {
  ...
  if (region.height >= 10) renderMeter("R", state.meterR, region.y + 9);
}
